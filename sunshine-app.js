(function () {
  ...
  function renderPage() {
    ...
    if (C.caseStudies) { setText('case-studies-kicker', C.caseStudies.kicker); setHeading('case-studies-title', C.caseStudies.titlePlain, C.caseStudies.titleHighlight); setText('case-studies-description', C.caseStudies.description); document.getElementById('case-studies-image')?.setAttribute('alt', C.caseStudies.imageAlt); document.getElementById('case-studies-grid').innerHTML = C.caseStudies.items.map(([title, desc], index) => `<article class="rounded-3xl border border-[var(--line)] bg-white p-6 shadow-sm sm:p-8"><div class="flex items-center justify-between gap-4"><span class="display text-4xl font-black text-gold/70">0${index + 1}</span><span class="rounded-full bg-mist px-3 py-1 text-xs font-bold text-navy">JoyLink Wire Mesh</span></div><h3 class="mt-7 text-xl font-black text-navy">${esc(title)}</h3><p class="mt-4 leading-7 text-body">${esc(desc)}</p></article>`).join(''); }
    ...
    document.getElementById('business-grid').innerHTML = C.business.map((item) => `<article class="group overflow-hidden rounded-3xl border border-[var(--line)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-soft">${item.image ? `<picture><source type="image/webp" srcset="/img/${esc(item.image)}.webp" /><img src="/img/${esc(item.image)}.jpg" alt="${esc(item.imageAlt || item.english)}" width="1200" height="675" loading="lazy" decoding="async" class="aspect-video w-full border-b border-[var(--line)] bg-navy object-cover" /></picture>` : ''}<div class="p-6 sm:p-8"><div class="flex items-start justify-between gap-4"><span class="display text-4xl font-black text-gold/70">${esc(item.index)}</span><span class="rounded-full bg-mist px-3 py-1 text-xs font-bold text-navy">${esc(item.english)}</span></div><h3 class="display mt-7 text-2xl font-black">${esc(item.title)}</h3><p class="mt-3 font-semibold text-navy">${esc(item.promise)}</p><p class="mt-4 leading-7 text-body">${esc(item.detail)}</p><div class="mt-7 grid gap-4 border-t border-[var(--line)] pt-5 sm:grid-cols-2"><div><div class="text-xs font-bold uppercase tracking-widest text-gold-deep">${esc(C.services.input)}</div><div class="mt-2 text-sm leading-6 text-body">${item.inputs.map(esc).join(' · ')}</div></div><div><div class="text-xs font-bold uppercase tracking-widest text-navy">${esc(C.services.output)}</div><div class="mt-2 text-sm leading-6 text-body">${item.outputs.map(esc).join(' · ')}</div></div></div></div></article>`).join('');
