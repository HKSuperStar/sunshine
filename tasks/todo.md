...
## 品牌视频合成（2026-08-25/26）

- [x] 5 条素材（01a~01e）逐条 0.5s 精细抽帧 + 分镜审阅（原计划 5 路并发 fork 受阻，改主线程直接执行）
- [x] 跨条冗余判定：看板/收尾卡/Agent-OS 场景各保留一次，01e 整条排除
- [x] 剪辑 + 4 处交叉淡化转场渲染，产出 `video/sunshine-brand-reel.mp4`（20.75s，静音）
- [x] 抽帧复核 5 段内部 + 4 处转场 + 片尾，确认无黑帧/花屏
- [ ] 是否接入网站页面 —— 待用户确认
